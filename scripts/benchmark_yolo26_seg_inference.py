...
"""
YOLO26 Segmentation Inference Benchmark - Three-Way Comparison
==============================================================
Compares segmentation inference performance across MLX GPU, PyTorch MPS, and PyTorch CPU backends.

Usage:
    python benchmark_yolo26_seg_inference.py
    python benchmark_yolo26_seg_inference.py --models n s      # Specific models only
    python benchmark_yolo26_seg_inference.py --runs 20         # More timed runs
    python benchmark_yolo26_seg_inference.py --output custom.json

Output:
    ../results/yolo26_seg_inference_three_way.json (default, overridable via --output)
"""
...
def test_mlx_inference(
    ...
    # Find YAML config in the package (yolo26-seg.yaml, not yolo26n-seg.yaml)
    try:
        import yolo26mlx

        pkg_dir = Path(yolo26mlx.__file__).parent
        yaml_candidates = [
            pkg_dir / "cfg" / "models" / "26" / "yolo26-seg.yaml",
            pkg_dir / "cfg" / "models" / "yolo26-seg.yaml",
        ]
    ...
    try:
        if npz_path.exists():
            ...
            model = YOLO(str(npz_path), task="segment")
            model_source = "npz"
        elif safetensors_path.exists():
            logger.info(f"  Loading {model_name} from safetensors...")
            model = YOLO(str(safetensors_path), task="segment")
            ...
            model = YOLO(str(yaml_path), task="segment")
...
def print_summary(results: dict[str, dict], speedups: dict[str, dict]) -> None:
    ...
    """
    logger.info("\n" + "=" * 80)
    logger.info("  YOLO26 Segmentation Inference Benchmark Summary")
    logger.info("=" * 80)

    # Header
    logger.info(
        f"\n{'Model':<14} {'MLX (ms)':<12} {'MPS (ms)':<12} {'CPU (ms)':<12} {'MLX vs MPS':<12} {'MLX vs CPU':<12}"
    )
    logger.info("-" * 74)
    ...
    for size in MODEL_SIZES:
        ...
        row_model = f"yolo26{size}-seg"
        logger.info(
            f"{row_model:<14} {mlx_str:<12} {mps_str:<12} {cpu_str:<12} {speedup_mps_str:<12} {speedup_cpu_str:<12}"
        )
    ...
    logger.info(f"\n{'Model':<14} {'MLX fwd':<12} {'MPS fwd':<12} {'MLX vs MPS':<12}")
    logger.info("-" * 50)
    ...
    for size in MODEL_SIZES:
        ...
        row_model = f"yolo26{size}-seg"
        logger.info(f"{row_model:<14} {mlx_str:<12} {mps_str:<12} {ratio_str:<12}")
...
def main():
    """Run the three-way segmentation inference benchmark, print results, and save to JSON."""
    logging.basicConfig(level=logging.INFO, format="%(message)s")

    parser = argparse.ArgumentParser(description="YOLO26 Segmentation Inference Benchmark")
    parser.add_argument(
        "--models",
        nargs="+",
        default=MODEL_SIZES,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=RESULTS_DIR / "yolo26_seg_inference_three_way.json",
        help="Output JSON path (default: results/yolo26_seg_inference_three_way.json)",
    )
    parser.add_argument(
        "--allow-random-mlx",
        action="store_true",
    ...
    logger.info("=" * 70)
    logger.info("  YOLO26 Segmentation Inference Benchmark - Three-Way Comparison")
    logger.info("=" * 70)
    ...
    output = {
        "benchmark": "YOLO26 Segmentation Inference - Three-Way Comparison",
