...
"""
YOLO26 PyTorch MPS Training Benchmark
======================================
Measures training performance using PyTorch MPS backend on Apple Silicon.

Usage:
    python benchmark_yolo26_training_mps.py
    python benchmark_yolo26_training_mps.py --models n s      # Specific models only
    python benchmark_yolo26_training_mps.py --epochs 5        # Fewer epochs
    python benchmark_yolo26_training_mps.py --batch 2         # Smaller batch size
    python benchmark_yolo26_training_mps.py --output custom.json

Output:
    ../results/yolo26_mps_training_final.json (default, overridable via --output)
"""
...
def train_model_mps(
    ...
    """Train single PyTorch model with MPS backend and measure time.
...
def main():
    """Parse CLI args, verify MPS support, train each model on the MPS backend, and save benchmark results."""
    logging.basicConfig(level=logging.INFO, format="%(message)s")

    parser = argparse.ArgumentParser(description="YOLO26 PyTorch MPS Training Benchmark")
    parser.add_argument(
        "--models",
        nargs="+",
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=RESULTS_DIR / "yolo26_mps_training_final.json",
        help="Output JSON path (default: results/yolo26_mps_training_final.json)",
    )
    args = parser.parse_args()
    ensure_runtime_dirs(PROJECT_DIR)

    logger.info("=" * 70)
    logger.info("YOLO26 PyTorch MPS Training Benchmark")
    logger.info("=" * 70)
    ...
    logger.info("\n✨ PyTorch MPS training benchmark complete!")
