...
"""
YOLO26 Segmentation Benchmark Results Collector
===============================================
...
- benchmark_yolo26_seg_inference.py       -> yolo26_seg_inference_three_way.json
- benchmark_yolo26_seg_training_mlx.py    -> yolo26_seg_mlx_training_final.json
- benchmark_yolo26_seg_training_mps.py    -> yolo26_seg_mps_training_final.json
- benchmark_yolo26_seg_training_cpu.py    -> yolo26_seg_cpu_training_final.json

And produces a combined report for analysis and chart generation.

Usage:
    python benchmark_yolo26_seg_collect_results.py
    python benchmark_yolo26_seg_collect_results.py --output custom_output.json
...
INPUT_FILES = {
    "inference": RESULTS_DIR / "yolo26_seg_inference_three_way.json",
    "training_mlx": RESULTS_DIR / "yolo26_seg_mlx_training_final.json",
    "training_mps": RESULTS_DIR / "yolo26_seg_mps_training_final.json",
    "training_cpu": RESULTS_DIR / "yolo26_seg_cpu_training_final.json",
}
    ...
    for backend_name, data in backends:
        ...
        for item in data.get("results", []):
            ...
            result[model_name][backend_name] = {
                ...
                "mAP50_mask": item.get("mAP50_mask", 0),
                "mAP50-95_mask": item.get("mAP50-95_mask", 0),
                "mAP50_box": item.get("mAP50_box", 0),
                "mAP50-95_box": item.get("mAP50-95_box", 0),
...
def print_inference_summary(inference_data: dict, speedups: dict) -> None:
    ...
    logger.info(
        f"\n{'Model':<14} {'MLX (ms)':<12} {'MPS (ms)':<12} {'CPU (ms)':<12} {'MLX vs CPU':<12} {'MLX vs MPS':<12}"
    )
    logger.info("-" * 80)
...
def print_training_summary(training_data: dict, speedups: dict) -> None:
    ...
    logger.info(
        f"\n{'Model':<14} {'MLX (s)':<12} {'MPS (s)':<12} {'CPU (s)':<12} {'MLX vs CPU':<12} {'MLX vs MPS':<12}"
    )
    logger.info("-" * 80)
...
def _mask_map50_for_display(backend: dict) -> float:
    v = backend.get("mAP50_mask", 0)
    if v:
        return float(v)
    return float(backend.get("mAP50", 0))


def _mask_map5095_for_display(backend: dict) -> float:
    v = backend.get("mAP50-95_mask", 0)
    if v:
        return float(v)
    return float(backend.get("mAP50-95", 0))


def print_accuracy_summary(training_data: dict) -> None:
    """Print model mask accuracy (mAP) summary table.

    Args:
        training_data: Normalized training data keyed by model name then backend, containing mAP scores.
    """
    logger.info("\n" + "=" * 80)
    logger.info("  ACCURACY SUMMARY (mask mAP after training)")
    logger.info("=" * 80)
    ...
    logger.info(
        f"\n{'Model':<14} {'MLX m50_m':<12} {'MLX m95_m':<12} {'MPS m50_m':<12} {'MPS m95_m':<12} {'CPU m50_m':<12} {'CPU m95_m':<12}"
    )
    logger.info("-" * 86)
    ...
    for size in MODEL_SIZES:
        ...
        mps = backends.get("pytorch_mps", {})
        cpu = backends.get("pytorch_cpu", {})

        mlx50 = _mask_map50_for_display(mlx)
        mlx95 = _mask_map5095_for_display(mlx)
        mps50 = _mask_map50_for_display(mps)
        mps95 = _mask_map5095_for_display(mps)
        cpu50 = _mask_map50_for_display(cpu)
        cpu95 = _mask_map5095_for_display(cpu)

        mlx50_s = f"{mlx50:.4f}" if mlx50 else "N/A"
        mlx95_s = f"{mlx95:.4f}" if mlx95 else "N/A"
        mps50_s = f"{mps50:.4f}" if mps50 else "N/A"
        mps95_s = f"{mps95:.4f}" if mps95 else "N/A"
        cpu50_s = f"{cpu50:.4f}" if cpu50 else "N/A"
        cpu95_s = f"{cpu95:.4f}" if cpu95 else "N/A"

        logger.info(
            f"{model_name:<14} {mlx50_s:<12} {mlx95_s:<12} {mps50_s:<12} {mps95_s:<12} {cpu50_s:<12} {cpu95_s:<12}"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="YOLO26 Segmentation Benchmark Results Collector")
    parser.add_argument(
        "--output",
        type=str,
        default=None,
        help="Output file path (default: ../results/yolo26_seg_benchmark_combined.json)",
    )
    args = parser.parse_args()
    ensure_runtime_dirs(PROJECT_DIR)

    logger.info("=" * 80)
    logger.info("  YOLO26 Segmentation Benchmark Results Collector")
    logger.info("=" * 80)
    ...
    if not any(raw_data.values()):
        ...
        logger.error("   - benchmark_yolo26_seg_inference.py")
        logger.error("   - benchmark_yolo26_seg_training_mlx.py")
        logger.error("   - benchmark_yolo26_seg_training_mps.py")
        logger.error("   - benchmark_yolo26_seg_training_cpu.py")
        return
    ...
    combined = {
        "benchmark": "YOLO26 Segmentation Combined Benchmark Results",
    ...
    output_path = (
        Path(args.output) if args.output else RESULTS_DIR / "yolo26_seg_benchmark_combined.json"
    )
